...
void imprime() {
    ...
    for (j = 0; j < 16; j++) {
        if (linha == 0 && coluna == j) {
            if (tela[1][j] == 0) {
                lcdChar((unsigned char) c1);
                continue;
            }
            if (tela[1][j] == bomba) {
                lcdChar((unsigned char) cb1);
                continue;
            }
        }
        if (tela[0][j] == bomba) {
            if (linha == 1 && coluna == j) {
                lcdChar((unsigned char) cb2);
                continue;
            }
            if (tela[1][j] == 0) {
                lcdChar((unsigned char) b1);
                continue;
            }
        }
        if (tela[0][j] == 0) {
            if (linha == 1 && coluna == j) {
                lcdChar((unsigned char) c2);
                continue;
            }
            if (tela[1][j] == 0) {
                lcdChar((unsigned char) p1);
                continue;
            }
            if (tela[1][j] == bomba) {
                lcdChar((unsigned char) b2);
                continue;
            }
        }
    }
...
unsigned int freeforall = 0, ffa = 0;

unsigned int read_LCD_buttons() {
    unsigned int button = kpRead();
